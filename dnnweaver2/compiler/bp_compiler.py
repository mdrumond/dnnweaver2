...
class BP_GraphCompiler(object):
	...
	self.log.debug('_conv_compile inputs:')
	self.log.debug(conv_op)
	self.log.debug(pu_op)
	self.log.debug(tiling)
	self.log.debug(array_n)
	self.log.debug(array_m)
	
        inst_array = []
        inst_array.append(SetupInstruction(16, 16).get_binary()) #setup op0 and op1 bitwidths
        ...
        for op in pu_op:
            ...
            if isinstance(op, MaxPooling):
                pool_pad = op.pad
		self.log.debug('op.pad: {}'.format(op.pad))
	...
	self.log.debug('data_addr: {}'.format(conv_op.data.fpga_addr))
	self.log.debug('weights_addr: {}'.format(conv_op.weights.fpga_addr))
	self.log.debug('bias_addr: {}'.format(conv_op.bias.fpga_addr))
	self.log.debug('output_addr: {}'.format(conv_op.output_tensors.fpga_addr))
        ...
        tensor_tile_shape = {
            ...
            ScratchPad.OBUF: (conv_op.output_tensors.fpga_shape[0],
                              ...
                              int(math.ceil(conv_op.output_tensors.fpga_shape[3]/float(array_n))), 
			      array_m),
            ScratchPad.WBUF: (int(math.ceil(conv_op.weights.fpga_shape[0]/float(array_n))),
                              ...
                              int(math.ceil(conv_op.weights.fpga_shape[3]/float(array_n))), 
			      array_n, 
			      array_m),
	...
	self.log.debug(tensor_tile_shape)

        #outer_loops
        num_outer_loops = 0
        for l, it in tiling.iteritems():
	    self.log.debug('OuterLoop:{}, {}'.format(l,it))
            if it[0] > 1:
		#A: 16-16 means array size?
		    ...
		    #self.log.debug(tensor)
                    shape = tensor_tile_shape[buf]
		    #self.log.debug(shape)
                    stride = (np.prod(shape[dim+1:]) * dim_stride * tensor.dtype.bits) / 8
		    self.log.debug('strides: buf:{} s:{} size:{}'.format(buf,s,stride))
                    if stride >= (1<<16):
    ...
    def compile(self, graph, acc_obj):
	    ...
	    self.log.debug('Intructions being written...')
            for inst in inst_array:
                f.write('{}'.format(inst))
                f.write('\n')
        #return np.array(inst_array, dtype=np.int32)
	return inst_array, stats
