...
def get_stats_fast(conv_params, tiling, order_type, verbose=False):
    ...
    """
    acc_obj, K, O, S, IC, OC, B, prec, im2col, energy_cost, _, _ = conv_params
    ...
    for loop in order_type:
        ...
        for namespace in writes:
		    ...
		    #print('loop:{} num_tiles:{} tile_size:{} namespace:{} cache_hit '.format(loop, num_tiles, tile_size, namespace ))
            else:
                writes[namespace] *= num_tiles
		#print('loop:{} num_tiles:{} tile_size:{} namespace:{}'.format(loop, num_tiles, tile_size, namespace ))
...
def optimize_for_order(conv_params, pool_kernel=None, pool_stride=None, sequential=True):
    ...
    if pool_stride is None:
        pool_stride = (1,1,1,1)
    conv_params_with_pool = acc_obj, K, O, S, IC, OC, B, prec, im2col, energy_cost, pool_kernel, pool_stride
    ...
    if not sequential:
	    ...
	    best_energy = None
            for r in results:
		tiling, order_type, cycles, energy, stats = r
	    ...
	    stats = get_stats_fast(conv_params_with_pool, best_tiling, best_order, verbose=True)
        ...
        best_order  = None
        for o in order:
            tiling, order_type, cycles,  energy, stats = _optimize_for_order(conv_params_with_pool, o)
...
def _optimize_for_order(conv_params, order_type, verbose=False):
    ...
    """
    acc_obj, K, O, S, IC, OC, B, prec, im2col, energy_cost, pool_kernel, pool_stride = conv_params
    I = (O - 1) * S + K
...
#                     print(tiling)
    ...
    return (best_tiling, order_type, best_cycles, best_energy, stats)

def optimize_for_order_bp(conv_params, pool_kernel=None, pool_stride=None, sequential=True):
    # Generate permutations for the order
    loops = ['B/b', 'OW/ow', 'OH/oh', 'IC/ic', 'OC/oc']
    order = set(permutations(loops))
    ...
    if not sequential:
        ...
        try:
            ...
            best_energy = None
            for r in results:
                tiling, order_type, cycles, energy, _ = r
                if best_cycles is None or best_cycles > cycles or (best_cycles == cycles and best_energy > energy):
                    best_cycles = cycles
		    best_energy = energy
                    best_tiling = tiling
	    ...
	    assert best_tiling != None, "No tiling fits the memory"
	    stats = get_stats_fast_bp(conv_params_with_pool, best_tiling, best_order, verbose=True)
        ...
        best_order  = None
        for o in order:
            tiling, order_type, cycles, energy, _ = _optimize_for_order_bp(conv_params_with_pool, o)
            if best_cycles is None or best_cycles > cycles or (best_cycles == cycles and best_energy > energy):
                best_cycles = cycles
                best_tiling = tiling
                best_order  = order_type
...
def _optimize_for_order_bp(conv_params, order_type, verbose=False):
    """
    For a given ordering, optimizes tiling
    Args:
        conv_params: A tuple with convolution params
        ...
        for _o in range(num_O_tiles):
            ...
            for _ic in range(num_IC_tiles):
                ...
                for _oc in range(num_OC_tiles):
                    ...
                    stats = get_stats_fast_bp(conv_params, tiling, order_type, verbose=verbose)
                    #break
                    if stats is None:
                        continue
...
def get_stats_fast_bp(conv_params, tiling, order_type, verbose=False):
    ...
    for loop in order_type:
        ...
        for namespace in writes:
            if rd_cache_hit[namespace]:
                if tile_deps_bp[loop][namespace]:
                    writes[namespace] *= num_tiles
                    rd_cache_hit[namespace] = False
            else:
                writes[namespace] *= num_tiles
...
def optimize_for_order_lstm(conv_params, pool_kernel=None, pool_stride=None, sequential=True):
    # Generate permutations for the order
    ...
    if not sequential:
        _bound_optimizer_method = functools.partial(_optimize_for_order_lstm, conv_params_with_pool)
	    ...
	    stats = get_stats_fast_lstm(conv_params_with_pool, best_tiling, best_order, verbose=True)
        ...
        best_order  = None
        for o in order:
            tiling, order_type, cycles,  energy, stats = _optimize_for_order_lstm(conv_params_with_pool, o)
            if best_cycles is None or best_cycles > cycles:
                best_cycles = cycles
                best_tiling = tiling
                best_order  = order_type
...
def _optimize_for_order_lstm(conv_params, order_type, verbose=False):
    """
    For a given ordering, optimizes tiling
    Args:
        conv_params: A tuple with convolution params
	...
	for _b in range(num_B_tiles):
		...
		for _o in range(num_O_tiles):
			...
			for _oc in range(num_OC_tiles):
			    ...
			    stats = get_stats_fast_lstm(conv_params, tiling, order_type, verbose=verbose)
			    #break
			    if stats is None:
				continue
    ...
    stats = get_stats_fast_lstm(conv_params, tiling, order_type, verbose=True)

    return (best_tiling, order_type, best_cycles, best_energy, stats)

def get_stats_fast_lstm(conv_params, tiling, order_type, verbose=False):
    """
    Returns cycles and memory accesses to DRAM, IBUF, OBUF, and WBUF
        TODOs: Without im2col, the calculation of weight and ibuf size is inexact
    """
    ...
    writes['wbuf'] = \
            ceil_a_by_b(ic, acc_obj.N) * acc_obj.N * kh * kw * \
            ceil_a_by_b(oc, acc_obj.M) * acc_obj.M * g * wprec
    ...
    for loop in order_type:
        ...
        for namespace in writes:
            if rd_cache_hit[namespace]:
                if tile_deps_lstm[loop][namespace]:
                    writes[namespace] *= num_tiles
                    rd_cache_hit[namespace] = False
		    #print('loop:{} num_tiles:{} tile_size:{} namespace:{} cache_hit '.format(loop, num_tiles, tile_size, namespace ))
            else:
        ...
        if verbose:
            ...
            logger.debug('\tMax write size: {}'.format(max_write_size))
            logger.debug('\tMax read size: {}'.format(max_read_size))
            logger.debug('\tLoop Dependencies: {}'.format(tile_deps_lstm[loop]))
            logger.debug('\tLoop Promote: {}'.format(rd_cache_hit))
            logger.debug('\tReads : {}'.format(reads))
            logger.debug('\tWrites: {}'.format(writes))
    ...
    if verbose:
	    logger.debug('Compute cycle per tile : {:>20,}'.format(acc_obj.get_compute_cycles(ic, oc, ow, oh, b, kw, kh, im2col)))
	    logger.debug('# of tiles : {:>20,}'.format(num_tiles))
    stats.compute_cycles = num_tiles * g * acc_obj.get_compute_cycles(ic, oc, ow, oh, b, kw, kh, im2col)
    stats.memory_cycles_required = ceil_a_by_b(stats.middle_dram_accesses, acc_obj.mem_if_width)
